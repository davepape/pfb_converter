...
pfNode* pfdLoadFile_pfa(const char *fileName)
{
	    ...
	    list_type >= 0 && list_type < L_COUNT)
	{
		...
		/* XXX change if you add more lists to the end, the order
		   depends not on the value but on the order of WRITE_LIST in
		   pfdStoreFile_pfa and pfdStoreFile_pfb */
                if(list_type == (L_SHADER))
		    break;
            }
	}
...
static void *pfa_read_ibr_tex(pfa_global_t *glb)
{
    ...
    pfIBRtextureNumRings(tex, n);

    for(i=0; i<n; i++)
    {
	READ_LINE();
	sscanf(glb->line_buf, "%d %g", &numTex, &horAngle);
        pfIBRtextureRing(tex, i, numTex, horAngle);
    }
...
static void *pfa_read_node(int node_num, pfa_global_t *glb)
{
	...
	case N_BILLBOARD:
		...
		READ_LINE();
		pfIBRnodeIBRtexture((pfIBRnode *)node, 
				    glb->rl_list[L_IBR_TEX][atoi(glb->line_buf)]);
...
static void *pfb_read_node(int node_num, pfa_global_t *glb)
{
	...
	case N_BILLBOARD:
		...
		pfIBRnodeIBRtexture((pfIBRnode *)node, 
				    glb->rl_list[L_IBR_TEX][buf[buf_pos++]]);
...
static void pfa_write_ibr_tex(pfIBRtexture *tex, pfa_global_t *glb)
{
    ...
    n = pfGetIBRtextureNumRings(tex);
    fprintf(glb->ofp, "%d\n", n);
    for(i=0; i<n; i++)
    {
        pfGetIBRtextureRing(tex, i, &numTex, &horAngle);
        fprintf(glb->ofp, "%d %g\n", numTex, horAngle);
    }
